...
async def _run_scrape_reviews_and_notify(payload: ScrapeRequest) -> None:
    try:
        ...
        print("sending result to webhook:", result[-100:])  # Print the last 100 characters of the result for debugging
        logger.info(f"\n\nscrape_reviews background: sending result to webhook: {result[-100:]}")
